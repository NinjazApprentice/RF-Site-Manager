...
st.set_page_config(page_title="RF Site Log", layout="wide")
...
with col_tools:
    ...
    if not df.empty:
        ...
        # Filters out already completed for the dropdown to keep it relevant
        pending_tasks = df[df["Status"] != "Completed"]
        
        if not pending_tasks.empty:
            task_idx = st.selectbox("Select Task to Update", pending_tasks.index, 
            ...
            if st.button("Confirm Update"):
                df.at[task_idx, "Status"] = new_status
                df.to_csv(DATA_FILE, index=False)
                st.success("Status Updated!")
                st.rerun()
        else:
            st.write("No pending tasks to update.")
        ...
        if st.button("Archive Completed Tasks"):
            # This saves a backup before deleting
            df.to_csv("work_log_backup.csv", index=False)
            df = df[df["Status"] != "Completed"]
            df.to_csv(DATA_FILE, index=False)
            st.warning("Completed tasks archived to backup.")
            st.rerun()
